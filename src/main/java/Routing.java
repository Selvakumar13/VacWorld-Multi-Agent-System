...
public class Routing extends Module {
    ...
    @TERM
    public long returnObstacleX(long X, long Y, String D, String location) {
        int x = (int) X;
        int y = (int) Y;
        ...
        // map.setObstacle(x+offset[0], y+offset[1]);
        // System.out.println("[" + agent.name() +"] Recording Obstacle: (" + (x+offset[0]) + "," + (y+offset[1]) + ") @ (" + x + "," + y+")");
        return (long) x+offset[0];
    }

    @TERM
    public long returnObstacleY(long X, long Y, String D, String location) {
        int x = (int) X;
        int y = (int) Y;
